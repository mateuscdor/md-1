...
handler.before = async function (m) {
    ...
    if (!(id in this.tekateki)) return this.sendButton(m.chat, 'Soal itu telah berakhir', wm, 'Lagi?', '.tekateki', m)
    if (m.quoted.id == this.tekateki[id][0].id) {
        let json = JSON.parse(JSON.stringify(this.tekateki[id][1]))
        if (['.tete', 'Bantuan', ''].includes(m.text)) return !0
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            global.db.data.users[m.sender].exp += this.tekateki[id][2]
            await this.sendButton(m.chat, benar + ` +${this.tekateki[id][2]} XP`, wm, 'Lagi?', '.tekateki', m)
