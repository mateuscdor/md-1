...
END:VCARD`.trim()
    ...
    */
    ...
    conn.relayWAMessage = async (pesanfull) => {
        await (pesanfull.message.audioMessage ? conn.sendPresenceUpdate('recording', pesanfull.key.remoteJid) : conn.sendPresenceUpdate('composing', pesanfull.key.remoteJid));
    ...
    */
    ...
    conn.sendStimg = async (jid, path, quoted, options = {}) => {
        ...
        let buffer
        buffer = await (options && (options.packname || options.author) ? writeExifImg(buff, options) : imageToWebp(buff));
    ...
    conn.sendStvid = async (jid, path, quoted, options = {}) => {
        ...
        let buffer
        buffer = await (options && (options.packname || options.author) ? writeExifVid(buff, options) : videoToWebp(buff));
        await conn.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
        return buffer
    }
